...
class emailManager(object):
    ...
    def __init__(self):
        ...
        if (os.path.exists(self.__downloadFolder__) != True):
    ...
    def __truncDatetime__(self, someDate):
    ...
    def __findEmails__(self):   
        ...
        typ, data = self.__M__.search(None, self.__emailSubject__)
        ...
        if (self.__maxDate__ > self.__logLastBillDate__) and ((len(x_byte) - 1) == self.__logNumberOfBills__):
            self.status[1] ='UpdateLast' 
            print("Last bill will be downloaded")
            self.__getEmailAttachment__(self.__maxDateEMailId__)
            print("Done")
        elif (self.__maxDate__ == self.__logLastBillDate__) and (len(x_byte) == self.__logNumberOfBills__):
